...
class MetadataViewer {
    ...
    addMetadataRows(tbody, sectionTitle, metadata, checkSensitive = false) {
        ...
        headerRow.innerHTML = `
            <td colspan="3" style="background: var(--light); font-weight: bold; padding: 1rem;">
                ${sectionTitle}
            </td>
        `;
        ...
        Object.entries(metadata).forEach(([key, value]) => {
            ...
            const isSensitive = checkSensitive && this.isSensitiveField(key);
            const riskLevel = isSensitive ? 
                '<span class="risk-high">⚠️ High Risk</span>' : 
